...
export default function Index() {
  ...
  const generateReport = async () => {
    ...
    // Helper to strip HTML, emojis, and clean text
    const cleanText = (html: string) => {
      ...
      text = text.replace(
        /[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\u{1F900}-\u{1F9FF}]|[\u{1FA70}-\u{1FAFF}]|[\u{2000}-\u{206F}]|[\u{2E00}-\u{2E7F}]|[\u{FE00}-\u{FE0F}]|[\u{200D}]|[\u{20D0}-\u{20FF}]/gu,
        "",
      );
    ...
    doc.setFontSize(14);
    doc.setTextColor(5, 150, 105);
    doc.text("Medical Analysis Results:", 20, yPosition);
    yPosition += 10;

    // Clean and format analysis text with proper structure
    ...
    sections.forEach((section) => {
      ...
      if (section.match(/^\d+\.\s|^Disclaimer:|^Doctor's Summary/)) {
        yPosition += 5; // Add extra space before sections
      ...
      const lines = doc.splitTextToSize(section.trim(), 170);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(new Date().toLocaleDateString(), 20, 290);
    }
