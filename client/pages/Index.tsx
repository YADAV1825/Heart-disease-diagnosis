...
export default function Index() {
  ...
  const generateReport = async () => {
    ...
    const addStyledText = (text: string, x: number, y: number, style: any = {}) => {
      ...
      doc.text(text, x, y);
      return y + (style.fontSize || 12) * 0.4 + 3;
    };
    ...
    // Header with styling
    doc.setFontSize(22);
    doc.setTextColor(0, 139, 139); // Medical teal
    ...
    doc.setFontSize(16);
    doc.setTextColor(59, 130, 246); // Medical blue
    ...
    const vitals = [
      ...
      `Heart Rate: ${formData.hr} BPM`,
      `SpO2: ${formData.spo2}%`,
      `Allergies: ${formData.allergies || "None reported"}`,
    ];

    vitals.forEach((vital) => {
      doc.text(vital, 25, yPosition);
      yPosition += 6;
    });
    yPosition += 10;
    ...
    doc.setFontSize(16);
    doc.setTextColor(59, 130, 246);
    doc.text("⚠️ Risk Factors", 20, yPosition);
    yPosition += 10;
    ...
    const riskFactors = [
      ...
      `Hypertensive Drugs: ${formData.hypertensive}`,
    ...
    lines.forEach((line: string) => {
      ...
      if (line.includes("Risk Score:") || line.includes("URGENT") || line.includes("Red Flags")) {
        doc.setTextColor(239, 68, 68); // Red for urgent items
        doc.setFont("helvetica", "bold");
      } else if (line.includes("Patient:") || line.includes("Assessment:")) {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text("Generated by CardioAI Screening System", 120, 285);
